...
for (const item of appendixItems) {
  ...
  for (let index = 0; index < parts; index += 1) {
    ...
    const body = `<div class="appendix-heading"><div class="label">${esc(label)}</div><h2>${esc(title)}</h2><p>${esc(item.label)} · 引文 ${index + 1}/${parts}${item.kind === 'edge' ? ` · 关系术语：${esc(item.edge.relation_terms.join('、'))}` : ''}</p></div><section class="quote-card citation-card">${quote(sliced, 700)}${citation(item.evidence)}</section>`;
