...
(() => {
  ...
  window.TipitakaV4.openEvidencePicker = function openEvidencePicker(options = {}) {
    ...
    return new Promise(resolve => {
      ...
      panel.innerHTML = `<div class="tipitaka-evidence-picker-inner"><button class="tipitaka-evidence-picker-close" aria-label="关闭">×</button><h3>${esc(options.title || '选择 V4 逐行证据')}</h3><p class="tipitaka-evidence-picker-note">仅检索 V4 三藏经文根层中的经藏与律藏正文，不包含义注、复注或阿毗达摩。每个候选都保留 work_id、row_id、语种、稳定 anchor、匹配高亮和阅读器深链接；选择后可在关系编辑器中使用。</p><form class="tipitaka-evidence-picker-form"><input id="tipitaka-evidence-picker-input" required placeholder="搜索中文、巴利或英文词组"><select id="tipitaka-evidence-picker-lang"><option value="zh">中文</option><option value="pali">巴利</option><option value="en">English</option></select><button class="tb-btn">搜索</button></form><div class="tipitaka-evidence-picker-status" id="tipitaka-evidence-picker-status">输入检索词开始。</div><div class="tipitaka-evidence-picker-results" id="tipitaka-evidence-picker-results"></div><div class="tipitaka-evidence-picker-pagination" id="tipitaka-evidence-picker-pagination"></div></div>`;
    ...
    const drawBookmarks = async () => {
      ...
      try {
        ...
        const groupsHtml = orderedWorkIds.map(workId => {
          ...
          const articles = items.map(item => `<article><a href="${esc(bookmarkHref(item))}"><strong>${esc(item.label || `${title} · ${item.row_id}`)}</strong><span>打开对应阅读位置 →</span></a><button type="button" data-t-bookmark-remove="${esc(item.work_id)}" data-t-bookmark-row="${esc(item.row_id)}" aria-label="移除此收藏">移除</button></article>`).join('');
